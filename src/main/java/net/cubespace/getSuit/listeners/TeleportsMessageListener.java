...
public class TeleportsMessageListener implements Listener {
    ...
    public void receivePluginMessage(PluginMessageEvent event) throws IOException, SQLException {
        ...
        if (task.equals("TeleportToLocation")) {
            ...
            TeleportManager.teleportPlayerToLocation(PlayerManager.getPlayer(sender), new Location(((Server) event.getSender()).getInfo().getName(), in.readUTF(), in.readDouble(), in.readDouble(), in.readDouble()));
            return;
        }
