...
import net.cubespace.getSuit.Utilities;
import net.cubespace.getSuit.objects.BSPlayer;
import net.cubespace.getSuit.objects.Ban;
import net.cubespace.getSuit.tasks.SendPluginMessage;
...
public class BansManager {
    ...
    public static boolean isPlayerBanned(String player) {
        return SQLManager.existanceQuery("SELECT player FROM BungeeBans WHERE player='" + player + "'");
    }
    ...
    public static void banPlayer(String bannedBy, String player, String reason) throws SQLException {
        ...
        BSPlayer t = PlayerManager.getPlayer(player);
        if (t != null) {
            player = t.getName();
        }
    ...
    public static void banIP(String bannedBy, String player, String reason) throws SQLException {
        ...
        for (String a : accounts) {
            ...
            if (PlayerManager.isPlayerOnline(a)) {
                disconnectPlayer(a, ConfigManager.messages.IPBAN_PLAYER.replace("{message}", reason).replace("{sender}", bannedBy));
            }
        }
        if (ConfigManager.bans.BroadcastBans) {
    ...
    public static void tempBanPlayer(String sender, String player, int minute, int hour, int day, String message) throws SQLException {
        BSPlayer p = PlayerManager.getPlayer(sender);
        BSPlayer t = PlayerManager.getPlayer(player);
        if (t != null) {
            player = t.getName();
