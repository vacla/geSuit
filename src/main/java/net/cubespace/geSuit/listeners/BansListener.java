...
public class BansListener implements Listener {
    ...
    public void banCheck(LoginEvent e) throws SQLException {
        if (DatabaseManager.bans.isPlayerBanned(e.getConnection().getName())) {
        ...
        if (DatabaseManager.bans.isPlayerBanned(e.getConnection().getAddress().getHostString().substring(1))) {
            Ban b = DatabaseManager.bans.getBanInfo(e.getConnection().getAddress().getHostString().substring(1));
            if (b.getType().equals("tempban")) {
                if (BansManager.checkTempBan(b)) {
                    e.setCancelled(true);
