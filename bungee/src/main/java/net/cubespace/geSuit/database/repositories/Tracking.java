...
public class Tracking extends BaseRepository {
    ...
    public Tracking(String name, String bansName, ConnectionPool pool) {
        super(name, pool);
        bansTable = bansName;
    }
    ...
    public void registerStatements() {
        ...
        nameHistory = registerStatement("nameHistory", "SELECT p1.`name`, p1.`nickname`, p1.`uuid`, p1.`ip`, p1.`firstseen`, p1.`lastseen`, 0 as `name_ban`, 0 as `ip_ban` FROM `%tracking%` p1 INNER JOIN (SELECT max(lastseen) lastseen, name FROM `%tracking%` WHERE uuid=? GROUP BY name,nickname) p2 ON p1.name = p2.name AND p1.lastseen = p2.lastseen WHERE p1.uuid=? ORDER BY p1.lastseen;".replace("%tracking%", getName()));
        checkNameChange = registerStatement("checkNameChange", "SELECT `name`, `nickname`, `uuid`, `ip`, `firstseen`, `lastseen`, 0 AS `name_ban`, 0 AS `ip_ban` FROM " + getName() + " WHERE `uuid`=? AND `name`!=? ORDER BY `lastseen` DESC LIMIT 1;");
        ...
        // Show the 25 most recent players whose name or nickname or uuid matches the search string
        matchPlayers = registerStatement("matchPlayers", "SELECT `uuid` FROM (SELECT `uuid`, `lastseen` FROM `%tracking%` WHERE `name` LIKE ? OR `nickname` LIKE ? OR `uuid` LIKE ? GROUP BY `uuid` ORDER BY lastseen desc LIMIT 25) AS FilterQ ORDER BY lastseen;".replace("%tracking%", getName()));
    }
    ...
    public Track getPlayerAlt(GlobalPlayer player) throws SQLException {
        ConnectionHandler handler = getConnection();
        ResultSet result = null;
        try {
            result = handler.executeQuery(getPlayerAlt, player.getAddress().getHostAddress(), Utilities.toString(player.getUniqueId()));
            if (result.next()) {
                return readTracking(result);
            } else {
                return null;
